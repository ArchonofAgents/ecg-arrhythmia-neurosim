...
parser = argparse.ArgumentParser(description='PyTorch Example for Multiple Datasets')
parser.add_argument('--parallelRead', type=int, default=128, help='number of rows read in parallel')
...
parser.add_argument('--log_interval', type=int, default=100, help='batches to wait before logging training status')
parser.add_argument('--test_interval', type=int, default=1, help='epochs to wait before testing')
...
parser.add_argument('--subArray', default=128, help='size of subArray')
parser.add_argument('--ADCprecision', default=5, help='ADC precision')
parser.add_argument('--cellBit', default=4, help='cell precision')
parser.add_argument('--onoffratio', default=10, help='device on/off ratio')
parser.add_argument('--vari', default=0, help='conductance variation')
parser.add_argument('--t', default=0, help='retention time')
parser.add_argument('--v', default=0, help='drift coefficient')
parser.add_argument('--detect', default=0, help='fixed/random drift')
...
if args.dataset == 'cifar10':
    ...
    train_loader, test_loader = dataset.get_arrhythmia_dataset(batch_size=args.batch_size, data_root='./data_root', train=True, val=True)
else:
...
if args.model == 'VGG8':
    ...
    if args.dataset == 'arrhythmia':
        model.classifier[-1] = torch.nn.Linear(in_features=model.classifier[-1].in_features, out_features=6)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if isinstance(data, tuple):
                data = data[0]
                print(f"Detected tuple in train loader. Extracted tensor with shape: {data.shape}")
    
            if args.dataset == 'arrhythmia':
                data = data.view(data.size(0), 1, 180, 1)  # Reshape for VGG compatibility
        ...
        if epoch % args.test_interval == 0:
            ...
            for i, (data, target) in enumerate(test_loader):
                if isinstance(data, tuple):
                    data = data[0]
                if args.dataset == 'arrhythmia':
                    data = data.view(data.size(0), 1, 180, 1)
                indx_target = target.clone()
                if args.cuda:
                    data, target = data.cuda(), target.cuda()
                with torch.no_grad():
